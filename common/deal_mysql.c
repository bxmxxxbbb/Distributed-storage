...
MYSQL *mysql_conn(char *user_name, char *passwd, char *db_name)
{
    ...
    // 用户分配或者初始化一个MYSQL对象，用于连接mysql服务端
    ...
    // NULL:值必须是主机名或者IP地址。如果值是NULL或字符串"loaclhost",连接将被是为与本地主机的连接
...
void process_result_test(MYSQL *conn, MYSQL_RES *res_set)
{
    ...
    // mysql_fetch_row从使用mysql_store_result得到的结果结构中提取一行，并把它放到一个行结构中，当数据用完或者发生错误时返回NULL
...
// 处理数据库查询结果，结果集保存在buf，只处理一条记录，一个字段，如果buf为NULL， 无需保存结果集，只做判断有没有此纪录
// 返回值： 0 成功并保存记录集，1没有记录集，2有记录集但是没有保存，-1 失败
int process_result_one(MYSQL *conn, char * sql_cmd, char buf)
{
    ...
    // mysql_num_rows接收由mysql_store_result返回的结果结构集，并返回结构集的行数
    ...
    else if(line > 0 && buf == NULL) //如果buf为NULL, 无需保存结果集，指挥判断有没有此记录
    {
        ret = 2;    // 2有记录但没有保存
        goto END;
    }

    //mysql_fetch_row从结果结构中提取一行，并把它放到一个行结构中。当数据用完胡发生错误时返回NULL
