...
/**
 * @brief   在字符串full_data中查找substr第一次出现的位置
 * 
...
/**
 * @brief 解析url query 类似 abc=123&&bb=456字符串
 *          传入一个key,得到相应的value
 * @returns 
 *          0 成功 -1 失败
*/
...
// 通过文件名file_name, 得到文件后缀字符串,保存在suffix 如果非法文件后缀,返回NULL
...
// 字符串strSrc中的字符串strFind,替换为sreReplace
    ...
    while (*strSrc != '\0')
    {
        if (*strSrc == * strFind)
        {
            if(strncmp(strSrc, strFind, strlen(strFind)) == 0)
            {
                ...
                char * temp = (char *)malloc(lastLen + 1);  //林没事开辟一段内存保存剩下的字符串,防止内存覆盖
                int k = 0;
                for( k = 0; k < lastLen; k++)
                {
                    *(temp + k) = *(p + k);
...
char *return_status(char *status_num)
{
    ...
    out = cJSON_Print(root);    //cjosn to string(char *)
    cJSON_Delete(root);
    return out;
}
