...
#define FIELD_ID_SIZE       100     /* redis hash 表field域最大长度*/
#define VALUES_ID_SIZE      1024    /*redis         value域最大长度*/
typedef char (*RCOMMANDS)[REDIS_COMMAND_SIZE]; /* redis 存放批量命令字符串数组类型*/
...
/*------------------------------------------------------------------------------------*/
/**
 * @brief tcp链接redis超时等待模式，timeval链接超时
 *                  返回
 * 
 * @param ip_str            redis服务器ip
 * @param port_str          redis服务器port
 ...
 * @param fields         hash表区域名称数组char(*)[FIELD_ID_SIZE]
...
*/
/*---------------------------------------------------------------------------------*/
int rop_hinecrement_one_field(redisContext *conn, char *key,char *field,unsigned int num);
...
/*--------------------------------------------------------------------------------*/
/**
 * @brief   向一个hash表中添加一条key-values 数据
 * @param   conn        已经建立好的链接
 * @param   key         hash表名
 * @param   field       
 * @param   values
...
/*-------------------------------------------------------------------------------*/
/**
 * @brief   设置key对应的值为string类型的value，同事设置过期时间
 * @param   conn    已建立好的链接
 * @param   key     key值
 ...
 *                  typedef char(*RVALUE)[VALUES_ID_SIZE]
