 ...
 *          0 成功 -1 失败 
*/
int query_pasre_key_value(const char *query, const char *key, char *value, int value_len_p);

// 通过文件名file_name，得到文件后缀字符串，保存在suffix 如果非法文件后缀，返回NULL
int get_file_suffix(const char *file_name,char *suffix);

// 字符串strSrc中的子串strFind，替换为strReplace
