...
int get_md5_info(char *buf, char *user, char *token, char *md5, char *filename)
{
    ...
    */
    ...
    //文件名字
    cJSON *child3 = cJSON_GetObjectItem(root, "filaName");
    if(child3 == NULL)
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "cJSON_GetObjectItem err\n");
        ret = -1;
...
int deal_md5(char *user, char *md5, char *filename)
{
    ...
    if(conn == NULL)
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "mysql_conn err\n");
        ret = -1;
        goto END;
    }
    ...
    */
    // 查看数据库是否有此文件的MD5
    // 如果没有，返回{"code":"006"},代表不能互传

    // 如果有
    // 1、修改file_info中的count字段，+1(count文件引用计数)
    // update file_info_set count = 2 where md5 = "bae488ee63cef72efb6a3f1f311b3743"
    // 2、user_file_list插入一条数据

    // sql语句、获取此md5值文件的文件计数器 count
    ...
    if(ret2 == 0)//有结果，说明服务器已经有此文件
    {
        ...
        //查看此用户是否已经拥有此文件，如果存在说明此文件已经上传，无需再次上传
        ...
        if(ret2 == 2)
        {
            ...
            ret = -2; //0秒成功，-1出错，-2此用户已拥有此文件，-3秒传失败
            goto END;
        }
        ...
        //使用函数gettimeofday()来得到时间。它的精度可以达到微秒
        gettimeofday(&tv, NULL);
        ptm = localtime(&tv.tv_sec);//把从1970-1-1零点零分到当前系统时间所偏移的秒数时间转换为本地时间
        ...
        --pv 文件下载量，默认值为0，下载一次+1
        */

        sprintf(sql_cmd, "insert into user_file_list(user, md5, create, filename, shared_status, pv) values ('%s', '%s','%s','%s',%d, %d)", user, md5, time_str, filename, 0, 0);
        if(mysql_query(conn, sql_cmd) != 0)
        {
            LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "%s 操作失败：%s\n",sql_cmd, mysql_error(conn));
            ret = -1;
        ...
        if(mysql_query(conn, sql_cmd) != 0)
        {
            LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "%s 操作失败： %s\n", sql_cmd,mysql_conn(conn));
            ret = -1;
            goto END;
        }

    }
    else if (ret2 == 1)// 没有结果，秒传失败
    {
