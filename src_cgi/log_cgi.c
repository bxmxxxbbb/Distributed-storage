...
int get_login_info(char *login_buf, char *user, char *pwd)
{
    ...
    if( child1 == NULL)
    {
        LOG(LOGIN_LOG_MODULE, LOGIN_LOG_PROC, "cJSON_GetOjectItem err\n");
        ret = -1;
        goto END;
    }
    ...
    //LOG(LOGIN_LOG_MODULE, LOGIN_LOG_PROC, "child2->valuestring = %s\n", child2->valuestring);
...
int check_user_pwd(char *user, char *pwd)
{
    ...
    //返回值：0成功并保存记录集，1、没有记录集，2、有记录集但是没有保存，-1失败
