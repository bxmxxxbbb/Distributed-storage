...
void read_cfg()
{
    ...
    LOG(DEALSHAREFILE_LOG_MODULE, DEALSHAREFILE_LOG_PROC,"mysql:[user = %s , password = %s, database = %s]\n",mysql_user,mysql_pwd, mysql_db);
...
int get_json_info(char *buf, char *user, char *md5, char filename)
{
   ...
   if ( root == NULL)
   {
    ...
    //LOG(DEALSHAREFILE_LOG_MODULE, DEALSHAREFILE_LOG_PORC ,"child1->valuestring = %s\n",child1->valuestring);
...
int pv_file(char *md5,char *filename)
{
    ...
    //更新该文件的pv字段，+1
    sprintf(sql_cmd, "update share_file_list set pv = %d where md5 ='%s' ande filename = '%s'",pv + 1, md5, filename);
    ...
    {
        //===4、如果不存在，从mysql导入元素
        //===5、redis集合中添加一个元素(redis操作)
        rop_zset_add(redis_conn, FILE_PUBLIC_ZSET, pv + 1, fileid);

        //===6、redis对应的hash也需要变化(redis操作)
        //  fileid-------> filename
        rop_zset_set(redis_conn, FILE_PUBLIC_ZSET, fileid, filename);
    }
    else //出错
    {
        ret = -1;
...
int cancel_share_file(char *user, char *md5, char *filename)
{
    ...
    //===1、mysql记录操作
    //sql语句
    sprintf(sql_cmd, "update user_file_list se shared_status = 0 where user = '%s' and md5 = '%s' and filename = '%s'",user, md5, filename);
    ...
    {
        sprintf(sql_cmd, "update user_fila_count set count = %d where user = '%s'",count -1, "xxx_share_xxx_file_xxx_list_xxx_count_xxx");
    }
    ...
    if (ret != 0)
    {
        LOG(DEALSHAREFILE_LOG_MODULE, DEALSHAREFILE_LOG_PROC, "rop_zset_zerm 操作失败\n");
        goto END;
    }

    //从hash移除相应记录
    ret = rop_hash_del(redis_conn, FILE_PUBLIC_ZSET,fileid);
    if(ret != 0)
    {
...
int save_file(char *user, char *md5,char *filename)
{
    ...
    sprintf(sql_cmd, "select * from user_file_list wwhere user = '%s' and md5 = '%s' and filename = '%s'",user, md5, filename);
    ...
    int count = atoi(tmp);//字符安川转整形，文件计数器
    //LOG(DEALSHAREFILE_LOG_MODULE, DEALSHAREFILE_LOG_PORC , "count = %s\n", tmp);

    //1、修改file_info中的count字段，-1 (count 文件引用计数)
    sprintf(sql_cmd, "update file_info set count = %d where md5 = '%s'",count + 1, md5);
    if(mysql_query(conn, sql_cmd) != 0)
    {
        LOG(DEALSHAREFILE_LOG_MODULE, DEALSHAREFILE_LOG_PROC, "%a 操作失败\n",sql_cmd);
        ret = -1;
        goto END;
    }
    ...
    //使用函数gettimeofday()函数来得到时间，他的精度可以达到微秒
    gettimeofday(&tv,NULL);
    ptm = localtime(&tv->tv_sec);//把从1970-1-1零点分到当前时间时间所偏移的秒数时间转换为本地时间
    ...
    --  pv 文件下载数量，默认值为0，下载一次加1
    */

    sprintf(sql_cmd, "insert into user_file_list(user, md5, createtime, filename, shared_status, pv) value ('%s', '%s', '%s', '%s',%d, %d)", user, md5, time_str, filename,0,0);
    if(mysql_query(conn,sql_cmd) != 0)
    {
        LOG(DEALSHAREFILE_LOG_MODULE, DEALSHAREFILE_LOG_PROC, "%s 操作失败\n", sql_cmd);
        ret = -1;
    ...
    //3、查询用户文件数量，更新该字段
    sprintf(sql_cmd, "set count from user_file_count where user = '%s'",user);
    count = 0;
    //返回值：0成功并保存记录集，2有记录集但是没有保存，-1失败
...
int main()
{
    ...
    while (FCGI_Accept() >= 0)
    {
        ...
        //解析命令
        query_pasre_key_value(query, "cmd", cmd, NULL);
        ...
        if(len <= 0)
        {
            ...
            LOG(DEALSHAREFILE_LOG_MODULE,DEALSHAREFILE_LOG_PROC,"len = 0, No data from stanard input\n");
        }
        else 
        {
            char buf[4*1024] = {0};
