...
// mysql 数据库配置信息配置信息  用户名，密码，数据库名称
...
void read_cfg()
{
    ...
    // LOG(MYFILES_LOG_MODULE,MYFILES_LOG_PROC,"redis:[ip:%s,port:%s]",redis_ip,redis_port);
}
...
// 按下载量降序 127.0.0.1:80/myfiles?cmd=pvdesc
int get_user_filelist(char *cmd, char *user, int start, int count)
{
    ...
    {
        // sql语句
        sprintf(sql_cmd, "select user_file_list.*, file_info.url, file_info.size, file_info.type from file_info, user_file_list where user = '%s' and file_indo.md5 = user_file_list.md5 order by pv asc limit %d, %d",user, start, count);
    }
    ...
    // mysql_num_rows接受由mysql_store_result返回结果集，并返回结果集中的行数
    ...
    // mysql_fetch_row从使用mysql_store_result得到的结果集构中提取一行，并把它放到第一个行结构中
    // 当数据使用完或发生错误时返回NULL
    while ((row = mysql_fetch_row(res_set)) != NULL)
    {
        ...
        //     "share_status":0,
        //     "pv":0,
        //     "url":"http://192.168.0.230:80/group1/M00/00/00/wKgfbViy2Z2AJ-FTAaM3As-g3Z0782.mp4",
        //     "size":27473666,
        //     "type":"mp4"
        // }
        ...
        // --shared_status 文件共享状态
        if(row[4] != NULL)
        {
            cJSON_AddNumberToObject(item, "share_Status", row[4]);
        }

        // --pv 文件下载量,默认值为0,下载一次加一
        if(row[5] != NULL)
        {
    ...
    cJSON_AddItemToObject(root, "file", array);
...
int main()
{
    ...
    while (FCGI_Accept() >= 0)
    {
        ...
        else 
        {
            ...
            {
                get_count_json_info(buf, user, token); //通过json包后去用户名，token
            ...
            else 
            {
                ...
                ret = verify_token(user, token); //util_chi.h
                if(ret == 0)
                {
