...
// 解析的json包
int get__filelist_json_info(char *buf, int *p_start, int *p_count)
{
    int ret = 0;
    /*
        "start" : 0
...
int get_share_filelist(int start, int count)
{
    ...
    // sql语句
    sprintf(sql_cmd, "select share_file_list.*, file_info.ul, file_info.size, file_info.type from file_info, share_file_list where file_info.md5 = share_file_list.md5 limit %d ,%d",start, count);
    ...
    while((row = mysql_fetch_row(res_set)) != NULL)
    {
        ...
        */
        ...
        // --sharead_status 共享状态，0为没有共享， 1为已经共享
...
{
    /*
        a)mysql_共享文件数量和redis共享问加你数量对比,判断是否相等
    ...
    */
    ...
    // ===3 mysql共享该文件数量和redis共享文件数量对比,判断是否相等
    if( redis_num != sql_num)
    {
        // ===4 如果不相等,清空redis,重新从mysql中导入数据到redis (mysql和redis交互)
        // a)清空redis有序数据
        ...
        // b)从mysql中导入数据到redis
        // sql语句
        strcpy(sql_cmd, "select md5, filename, pv, share_file_list order bu pv desc");
        ...
        unsigned long line = 0;
        // mysql_num_rows接受由mysql_store_result返回的结果结构集，并返回结果集中的行数
        line = mysql_num_rows(res_set);
        if(line == 0)
        {
            LOG(SHAREFILES_LOG_MODULE, SHAREFILES_LOG_PROC, "mysql_num_rows(res_set) failed\n");
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        */
        ...
        // --pv 文件下载数量
        int score = rop_zset_get_socre(redis_conn, FILE_PUBLIC_ZSET, value[i]);
        if(score == -1)
        {
...
int main()
{
    ...
    while (FCGI_Accpet() >= 0)
    {
        ...
        printf("Content-type:text/htl\r\n\r\n");

        if(strcmp(cmd, "count") == 0) // count获取用户文件个数
        {
        ...
        else
        {
            ...
            else
            {
                ...
                // 按下载量降序 127.0.0.1:80/sharefiles?cmd=pvdesc

                int start; //文件起点
                int count; //文件个数
                get_filelist_json_info(buf, &start, &count); //通过json包获取信息
                LOG(SHAREFILES_LOG_MODULE, SHAREFILES_LOG_PROC, "start = %d, count = %d, count = %d\n", start, count);
                ...
                {
                    get_ranking_filelist(start, count); //获取共享文件排行榜
                }
            }
        }
    }
