...
void read_cfg()
{
    ...
    LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "mysql[user = %s, password = %s, database = %s\n]", mysql_user, mysql_pwd, mysql_db);

    // 读取redis配置信息
    // get_cfg_value(CFG_PATH, "redis", "ip", redis_ip);
    // get_cfg_value(CFG_PATH, "redis", "port", redis_port);
    // LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC,"redis[ip = %s, port = %s]", redis_ip, redis_port);
}
...
int recv_save_file(long len, char *user, char *filename, char *md5, long *p_size)
{
    ...
    if(file_buf == NULL)
    {
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "malloc error! file size is to big!!!!\n");
        return -1;
    }
    ...
    if(ret2 == 0)
    {
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "fread(file_buf, 1, len, stdin) err\n");
        ret = -1;
        goto END;
    }
    ...
    /*
        -----WebKitFormBoundary88asdgewx\r\n
        Content-Disposition: from-data: user= \"mike"; filename="xxx.jpg"; md5="xxxx"; size=10240\r\n
    ...
    */
    ...
    // get boundary 得到分界线，-----WebKitFormBoundary88gewx
    p = strstr(begin, "\r\n");
    if( p == NULL)
    {
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "wrong no boundary!\n");
        ret = -1;
         goto END;
    }
    ...
    boundary[p-begin] = '\0'; //结束字符串
    // LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "boundary:[%s]\n", boundary);
    ...
    // Content-Disposition: from-data; user:"mike"; filenam = "xxx.jpg"; md5 = "xxx"; size = 10240\r\n
    p = strstr(begin, "\r\n");
    if(p == NULL)
    {
        LOG ( UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "ERROR: get context text error, no filename?\n");
        ret = -1;
        goto END;
    }
    strncpy(content_text, begin, p-begin);
    content_text[p - begin] = '\0';
    // LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "content_text: [%s]\n", content_text);
    ...
    // xxx.jpg"; md5="xxxx"; size=10240\r\n

    k = strstr(q, '"');
    strncpy(filename, q, k - q); //拷贝文件名
    filename[k-q] = '\0';

    trim_space(filename); // 去除首位空白字符
    ...
    */
    ...
    begin = p; 
    // find file's end
    p = memstr(begin, len, boundary); // util_cgi.h 文件结尾
    if( p == NULL )
    {
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "memstr(begin, len, boundary) error\n");
        ret = -1;
        goto END;
    }
    else 
    ...
    // ======>将数据写入文件中，其中文件名也是从post数据解析的来<=======
    
    int fd = 0;
    fd = open(filename, O_CREAT|O_WRONLY, 0644);
    if(fd < 0)
    {
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "open %s error\n", filename);
        ret = -1;
        goto END;
    }
...
int upload_to_dstorage(char *filename, char *fileid)
{
    ...
    if(pipe(fd) < 0)
    {
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "pip error\n");
        ret = -1;
        goto END;
    }
    ...
    if(pid < 0) //创建进程失败
    {
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "fork error\n");
        ret = -1;
        goto END;
    }
    ...
    if(pid == 0) //子进程
    {
        ...
        // 执行失败
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "execlp fdfs_upload_file error\n");
    ...
    else //父进程
    {
        ...
        if(strlen(fileid) == 0)
        {
            LOG( UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "[upload FAILED!]\n");
            ret = -1;
            goto END;
        }

        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "get [%s] succ!\n", fileid);

        wait(NULL); //等待资金正结束,回收器资源
        close(fd[0]);
    }
...
int make_file_url(char *fileid, char *fdfs_file_url)
{
    ...
    // printf("[%s]\n", fdfs_file_url);
    LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "file url is: %s\n", fdfs_file_url);
...
int store_fileinfo_to_mysql(char *user, char *filename, char *md5, long size, char *fileid, char *fdfs_file_url)
{
    ...
    if(conn == NULL)
    {
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "mysql_conn err\n");
        ret = -1;
        goto END;
    }
    // 设置数据库编码
    ...
    /*
        ...
        -- url 文件url
        -- size 文件大小，以字节为单位
        -- type 文件类型: png, zip, m94...
        -- count 文件引用计数， 默认为1， 每增加一个用户拥有此文件，此计数器+1
    */

    sprintf(sql_cmd, "insert into file_info (md, file_id, url, size, type, count) values ('%s', '%s', '%s', ;%ld', '%s', %d)", md5, fileid, fdfs_file_url, size, suffix, 1);

    if(mysql_query(conn, sql_cmd) != 0) //执行sql语句
    {
        // print_error(conn, "插入失败\n");
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "%s 插入失败 %s\n", sql_cmd, mysql_error(conn));
        ret = -1;
        goto END;
    }

    LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "%s 文件信息插入成功\n", sql_cmd);

    // 获取档前时间
    now = time(NULL);
    strfime(create_time, TIME_STRING_LEN - 1, "%Y-%m-%d %H:%M:%S", localtime(&now));
    ...
    /*
        ...
        -- createtime 文件创建时间
        -- filename 文件名字
        -- shared_status 共享文件状态， 0为没有， 1为共享
        -- pv 文件下载量，默认值问0， 下载一次+1
    */
    // sql语句
    ...
    {
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "%s 操作失败：%s\n", sql_cmd, mysql_error(conn));
        ret = -1;
        goto END;
    }
...
int main()
{
    ...
    char fdfs_file_url[FILE_URL_LEN] ={0};      //文件存放storage的host_name
    ...
    {   
        ...
        if(len <= 0)
        {
            ...
            LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "len = 0, No data from standard input\n");
            ret = -1;
        }
        else 
        {
            ...
            LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PORC, "%s成功上传[%s, 大小：%ld， md5码： %s]到本地\n",user, filename, size, md5);
