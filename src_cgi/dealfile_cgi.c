...
int get_json_info(char *buf,char *user, char *token , char *md5,char *filename)
{
   ...
   if(NULL == child1)
   {
        LOG(DEALFILE_LOG_MODULE,DEALFILE_LOG_PROC,"cJSON_GetOjectItem err\n");
        ret = -1;
         goto END;
   }
   ...
   //文件名字
   cJSON *child3 = cJSON_GetObjectItem(root, "filenname");
   if (NULL == child3)
   {
        LOG(DEALFILE_LOG_MODULE,DEALFILE_LOG_PROC,"cJSON_GetOjectItem err\n");
        ret = -1;
...
{
    /*
    a)先判断此文件是否已经被分享，判断集合有没有这个文件，如果有，说明别人已经分享此文件，中断操作(redis操作)
    b)如果集合有此元素，可能因为redis中没有记录，再从mysql中查询，如果mysql也没有，说明真没有(mysql操作)
    c)如果mysql没有记录，而redis没有记录，说明redis没有保存此文件，redis保存文件信息后，在中断操作(redis操作)
   ...
   if( redis_conn == NULL)
   {
    ...
    //===1、先判断此文件是否已经被分享，判断集合有没有这个文件，如果有，说明别人已经分享此文件，中断操作(redis操作)
    ...
    {
        //==2如果集合没有此元素，可能因为redis中没有记录，再从mysql中查询，如果mysql也没有，说明真没有(mysql操作)
        //==3如果mysql没有记录，而redis没有记录，说明redis没有保存此文件，redis保存文件信息后，在中断操作(redis操作)

        //查看此文件别人受否已经分享了
        sprintf(sql_cmd, "select * from share_file_list where md5 = '%s' and filename = '%s'",md5, filename);

        //返回值：0成功并保存记录集，1没有记录集， 2有记录但没有保存，-1失败
    ...
    //==4、如果此文件没有被分享，mysql保存一本持久化操作(mysql操作)
    //sql语句，更新共享标示字段
    ...
    //获取时间
    now = time(NULL);
    strftime(create_time,TIME_STRING_LEN-1, "%Y-%m%=%d %H:%M:%S",localtime(&time));

    // 分享文件的信息，额外保存再share_file_list保存列表
    /*
        --user 文件所属用户
        --md5  文件MD5
        --createtime  文件共享时间
    ...
    */
    ...
    //共享文件数量
    sprintf(sql_cmd,"select count from user_file_count where user = '%s'","xxx_share_xxx_file_xxx_list_xxx_count_xxx", 1);
    int count = 0;
    ...
    {
        //插入记录
        sprintf(sql_cmd, "update info user_file_count (user, count) values('%s', %d)","xxx_share_xxx_file_xxx_count_xxx", 1);
    }
    else if(ret2 == 0){
        //更新用户文件数量count字段
        count = atoi(tmp);
...
int del_file(char *user,char *md5,char *filename)
{
    /*
        ...
        d)如果mysql有记录，而redis没有记录，那么分享文件处理只需要处理mysql(mysql)
    ...
    */
    ...
    //===1、先判断此文件是否已经被分享，判断集合有没有这个文件，如果有，说明别人已经分享此文件
    ret2 = rop_zset_exit(redis_conn, FILE_PUBLIC_ZSET, fileid);
    if(ret2 == 1)//存在
    {
        share = 1; //共享标志
    ...
    else if(ret2 == 0) //不存在
    {
        ...
        //返回值： 0成功并保存记录集，1没有记录集，2有记录但是没有保存，-1失败
        ret2 = process_result_one(conn , sql_cmd, tmp);//执行sql语句
        if(ret2 == 0)
        {
    ...
    //说明此文件被分享，删除分享文件列表(shared_file_list)的数据
    if(share == 1)
    {
...
int pv_file (char *user, char *md5, char *filename)
{
    ...
    //更新该文件pv字段，+1
    sprintf(sql_cmd, "update user_file_list set pv = %d where user = '%s and md5 = '%s' and filename = '%s'",pv, user, md5, filename);
...
int main()
{
    ...
    while(FCGI_Accept() >= 0)
    {
        ...
        if( len <= 0)
        {
            ...
            LOG(DEALFILE_LOG_MODULE,DEALFILE_LOG_PROC, "len = 0, No data from standardinput\n");
        }
        else 
        {
            char buf[4*1024] = {0};
            ...
            LOG(DEALFILE_LOG_MODULE, DEALFILE_LOG_PROC, "user = %s, md5 = %s, filename = %s\n",user,md5,filename);
